#ifndef TWRP_LVGL_DEMO_I18N_H
#define TWRP_LVGL_DEMO_I18N_H

namespace gui2_i18n {

enum class language_id {
  ENGLISH,
  ZH_CN,
  ZH_TW,
};

struct action_text {
  const char* title;
  const char* summary;
  const char* detail;
};

struct language_pack {
  const char* native_name;
  const char* home_summary;
  const char* language_title;
  const char* language_summary;
  const char* current_language_detail;
  const char* screen_title;
  const char* screen_summary;
  const char* brightness_label;
  const char* screen_timeout_label;
  const char* screen_timeout_never;
  const char* haptics_title;
  const char* haptics_summary;
  const char* button_haptics;
  const char* keyboard_haptics;
  const char* action_haptics;
  const char* hardware_error;
  const char* apply;
  const char* time_title;
  const char* time_summary;
  const char* time_format;
  const char* twelve_hour;
  const char* twenty_four_hour;
  const char* select_timezone;
  const char* timezone_offset;
  const char* offset_none;
  const char* offset_15;
  const char* offset_30;
  const char* offset_45;
  const char* use_dst;
  const char* current_timezone;
  const char* classic_gui_title;
  const char* classic_gui_detail;
  const char* classic_gui_confirm_title;
  const char* classic_gui_confirm_body;
  const char* classic_gui_confirm;
  const char* cancel;
  const char* screenshot;
  const char* screenshot_saved;
  const char* screenshot_failed;
  const char* screen_off;
  const char* screen_on;
  const char* swipe_to_unlock;
  const char* start_recording;
  const char* stop_recording;
  const char* recording_started;
  const char* recording_saved;
  const char* recording_failed;
  const char* recording_indicator;
  const char* recording_settings_title;
  const char* recording_settings_summary;
  const char* recording_fps_label;
  const char* quick_menu_title;
  const char* reboot_title;
  const char* reboot_summary;
  const char* reboot_system;
  const char* reboot_power_off;
  const char* reboot_recovery;
  const char* reboot_fastboot;
  const char* reboot_bootloader;
  const char* reboot_download;
  const char* reboot_edl;
  const char* current_boot_slot;
  const char* boot_slot_a;
  const char* boot_slot_b;
  const char* swipe_reboot;
  const char* swipe_power_off;
  const char* reboot_failed;
  const char* console_title;
  const char* console_summary;
  const char* console_empty;
  const char* console_settings_title;
  const char* console_settings_summary;
  const char* console_font_label;
  const char* console_font_steps[3];
  const char* export_log_title;
  const char* export_log_summary;
  const char* include_kernel_log;
  const char* include_logcat;
  const char* export_log;
  const char* export_log_done;
  const char* export_log_failed;
  const char* wipe_title;
  const char* wipe_summary;
  const char* factory_reset_detail;
  const char* swipe_factory_reset;
  const char* advanced_wipe_title;
  const char* advanced_wipe_summary;
  const char* dalvik_cache;
  const char* swipe_wipe;
  const char* format_data_title;
  const char* format_data_summary;
  const char* format_data_warning;
  const char* format_data_prompt;
  const char* swipe_format_data;
  const char* wiping;
  const char* wipe_complete;
  const char* wipe_failed;
  const char* decrypt_title;
  const char* decrypt_summary;
  const char* decrypt_pattern_prompt;
  const char* decrypt_password_prompt;
  const char* decrypt_failed;
  const char* decrypting;
  const char* decrypt_complete;
  const char* backup_title;
  const char* backup_summary;
  const char* backup_partitions_tab;
  const char* backup_options_tab;
  const char* backup_name_label;
  const char* backup_compress;
  const char* backup_skip_digest;
  const char* swipe_backup;
  const char* backing_up;
  const char* backup_complete;
  const char* backup_failed;
  const char* backup_cancelled;
  const char* mount_title;
  const char* mount_summary;
  const char* mount_system_writable;
  const char* mount_system_note;
  const char* refreshing_sizes;
  const char* refresh_sizes_done;
  const char* backup_encrypt;
  const char* backup_password;
  const char* change_language;
  const char* data_encrypted_notice;
  const char* keyboard_copy;
  const char* keyboard_paste;
  const char* restore_title;
  const char* restore_summary;
  const char* restore_choose;
  const char* restore_none;
  const char* restore_encrypted_notice;
  const char* swipe_restore;
  const char* restoring;
  const char* restore_complete;
  const char* restore_failed;
  const char* restore_verify_digest;
  const char* restore_wrong_password;
  const char* mount_storage_section;
  const char* mount_partitions_section;
  const char* mount_other_section;
  const char* mount_mtp;
  const char* mount_usb_storage;
  const char* mount_decrypt_data;
  const char* mount_decrypt_data_detail;
  const char* select_storage_title;
  const char* select_storage_summary;
  const char* select_storage_current;
  const char* install_pick;
  const char* install_zip_tab;
  const char* install_image_tab;
  const char* install_confirm;
  const char* swipe_install;
  const char* installing;
  const char* install_complete;
  const char* install_failed;
  const char* install_target;
  const char* install_both_slots;
  const char* install_none;
  const char* install_warning;
  const char* install_folder;
  const char* install_file;
  const char* install_options;
  const char* install_queue;
  const char* install_queue_count;
  const char* install_add_zip;
  const char* install_clear_queue;
  const char* fm_new_name;
  const char* fm_new_mode;
  const char* fm_choose_action;
  const char* fm_selected_file;
  const char* fm_selected_folder;
  const char* fm_open_terminal;
  const char* fm_copy;
  const char* fm_move;
  const char* fm_chmod755;
  const char* fm_chmod;
  const char* fm_rename;
  const char* fm_delete;
  const char* fm_delete_confirm;
  const char* fm_destination;
  const char* fm_failed;
  const char* fm_this_folder;
  const char* fm_parent;
  const char* file_manager_title;
  const char* file_manager_summary;
  const char* file_manager_empty;
  const char* console_tab_output;
  const char* console_tab_terminal;
  const char* terminal_empty;
  const char* terminal_hint;
  const char* terminal_unavailable;
  const char* terminal_summary;
  const char* wifi_title;
  const char* wifi_summary;
  const char* wifi_networks;
  const char* wifi_scan;
  const char* wifi_scanning;
  const char* wifi_none;
  const char* wifi_open;
  const char* wifi_password_title;
  const char* wifi_password_hint;
  const char* wifi_connecting;
  const char* wifi_connected;
  const char* wifi_failed;
  const char* wifi_status;
  const char* wifi_test;
  const char* wifi_service;
  const char* wifi_log;
  const char* startup_system;
  const char* startup_scripts;
  const char* startup_services;
  const char* startup_finishing;
  const char* startup_unlock;
  const char* ors_title;
  const char* ors_running;
  const char* ors_done;
  const char* sys_ro_title;
  const char* sys_ro_summary;
  const char* sys_ro_body;
  const char* sys_ro_never_show;
  const char* sys_ro_keep;
  const char* sys_ro_swipe;
  const char* sideload_title;
  const char* sideload_summary;
  const char* sideload_hint;
  const char* sideload_usage;
  const char* sideload_wipe_dalvik;
  const char* sideload_wipe_cache;
  const char* sideload_swipe;
  const char* sideload_waiting;
  const char* sideload_complete;
  const char* sideload_failed;
  const char* sideload_cancelled;
  const char* fastbootd_title;
  const char* fastbootd_summary;
  const char* usb_mode;
  const char* repair_change_title;
  const char* repair_change_hint;
  const char* repair_change_invalid;
  const char* part_options;
  const char* part_mount_point;
  const char* part_file_system;
  const char* part_present;
  const char* part_removable;
  const char* part_size;
  const char* part_used;
  const char* part_free;
  const char* part_backup_size;
  const char* answer_yes;
  const char* answer_no;
  const char* repair_fs;
  const char* resize_fs;
  const char* change_fs;
  const char* change_fs_summary;
  const char* change_fs_warning;
  const char* change_fs_new;
  const char* fs_current;
  const char* swipe_change_fs;
  const char* repair_confirm;
  const char* resize_confirm;
  const char* swipe_repair;
  const char* swipe_resize;
  const char* repairing;
  const char* repair_complete;
  const char* resizing;
  const char* resize_complete;
  const char* formatting;
  const char* format_complete;
  const char* operation_failed;
  const char* manage_backup;
  const char* rename_backup;
  const char* delete_backup;
  const char* backup_name_exists;
  const char* backup_name_invalid;
  const char* rename_backup_confirm;
  const char* swipe_rename;
  const char* renaming_backup;
  const char* rename_backup_complete;
  const char* delete_backup_body;
  const char* swipe_delete_backup;
  const char* deleting_backup;
  const char* backup_deleted;
  const char* restore_lock_tip;
  const char* multiuser_body;
  const char* twrp_folder_title;
  const char* twrp_folder_current;
  const char* twrp_folder_hint;
  const char* folder_name_invalid;
  const char* folder_exists;
  const char* restore_default_folder;
  const char* restore_default_folder_detail;
  const char* twrp_folder_confirm;
  const char* changing_twrp_folder;
  const char* twrp_folder_changed;
  const char* fix_bootloop_title;
  const char* fix_bootloop_summary;
  const char* fixing_bootloop;
  const char* fix_bootloop_complete;
  const char* merge_title;
  const char* merge_summary;
  const char* merging;
  const char* merge_complete;
  const char* avb_title;
  const char* avb_summary;
  const char* disabling_avb;
  const char* avb_complete;
  const char* swipe_confirm;
  const char* keyboard_settings_title;
  const char* keyboard_settings_summary;
  const char* keyboard_lift_label;
  const char* general_settings_title;
  const char* general_settings_summary;
  const char* general_group_install;
  const char* general_group_wipe_backup;
  const char* general_zip_signature;
  const char* general_skip_digest_zip;
  const char* general_install_reboot;
  const char* general_unmount_system;
  const char* general_disable_avb2;
  const char* general_reflash_twrp;
  const char* general_rm_rf;
  const char* general_disable_free_space;
  const char* general_skip_digest_generate;
  const char* general_verify_digest;
  const char* general_use_sha2;
  const char* general_restore_defaults;
  const char* action_back;
  const char* action_reboot_system;
  const char* timezone_names[24];
  action_text actions[7];
};

const language_pack& get_language_pack(language_id language);

}  // namespace gui2_i18n

#endif  // TWRP_LVGL_DEMO_I18N_H
